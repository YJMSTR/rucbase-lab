...
TEST_F(BufferPoolManagerTest, SimpleTest) {
    ...
    auto *page0 = bpm->NewPage(&tmp_page_id);
    ...
    for (size_t i = buffer_pool_size; i < buffer_pool_size * 2; ++i) {
        EXPECT_EQ(nullptr, bpm->NewPage(&tmp_page_id));
    }
...
TEST_F(BufferPoolManagerTest, LargeScaleTest) {
    ...
    for (int i = 0; i < scale / buffer_pool_size; i++) {
        for (int j = 0; j < buffer_pool_size; j++) {
            auto new_page = bpm->NewPage(&tmp_page_id);
            EXPECT_NE(nullptr, new_page);
            strcpy(new_page->GetData(), std::to_string(tmp_page_id.page_no).c_str());
            page_ids.push_back(tmp_page_id);
        }
        for (unsigned int j = page_ids.size() - buffer_pool_size; j < page_ids.size(); j++) {
            EXPECT_EQ(true, bpm->UnpinPage(page_ids[j], true));
        }
    }
    ...
    for (int i = 0; i < scale; i++) {
        EXPECT_EQ(true, bpm->DeletePage(page_ids[i]));
    }
...
TEST_F(BufferPoolManagerTest, MultipleFilesTest) {
    ...
    for (auto &fh : mock) {
        ...
        for (page_id_t i = 0; i < MAX_PAGES; i++) {
            ...
            EXPECT_EQ(memcmp(page->GetData(), mock_buf, PAGE_SIZE), 0);

            bool unpin_flag = buffer_pool_manager->UnpinPage(page->GetPageId(), true);  // unpin the page
            EXPECT_EQ(unpin_flag, true);
        }
    }

    /** Test FlushAllPages(), FetchPage(), UnpinPage() */
    // Flush and test disk
    for (auto &entry : fd2name) {
        int fd = entry.first;
        buffer_pool_manager->FlushAllPages(fd);  // wirte all pages in fd file into disk
        for (int page_no = 0; page_no < MAX_PAGES; page_no++) {
            ...
            EXPECT_EQ(memcmp(buf, page->GetData(), PAGE_SIZE), 0);
            bool unpin_flag = buffer_pool_manager->UnpinPage(page->GetPageId(), false);
            EXPECT_EQ(unpin_flag, true);
        }
    }
...
TEST_F(BufferPoolManagerTest, ConcurrencyTest) {
    ...
    for (int run = 0; run < num_runs; run++) {
        ...
        for (int i = 0; i < buffer_pool_size; i++) {
            auto *new_page = bpm->NewPage(&tmp_page_id);
            EXPECT_NE(nullptr, new_page);
            EXPECT_EQ(true, bpm->UnpinPage(tmp_page_id, true));
        }

        for (int j = 0; j < buffer_pool_size; j++) {
            auto *page = bpm->FetchPage(page_ids[j]);
            EXPECT_NE(nullptr, page);
            strcpy(page->GetData(), (std::string("Hard") + std::to_string(page_ids[j].page_no)).c_str());
        }
            ...
            threads.push_back(std::thread([&bpm, tid, page_ids, fd]() {
                ...
                while (j < buffer_pool_size) {
                    if (j != tid * 10) {
                        ...
                        EXPECT_EQ(true, bpm->UnpinPage(temp_page_id, false));
                        // If the page is still in buffer pool then put it in free list,
                        // else also we are happy
                        EXPECT_EQ(true, bpm->DeletePage(temp_page_id));
                    }
                    ...
                    if (j % 2 == 0) {
                        ...
                        EXPECT_EQ(true, bpm->UnpinPage(page_ids[j], false));
                    } else {
                    ...
                    EXPECT_NE(nullptr, page);
                    strcpy(page->GetData(), std::to_string(temp_page_id.page_no).c_str());
                    // FLush page instead of unpining with true
                    EXPECT_EQ(true, bpm->FlushPage(temp_page_id));
                    EXPECT_EQ(true, bpm->UnpinPage(temp_page_id, false));
                    ...
                    for (int k = 0; k < 10; k++) {
                        ...
                        EXPECT_EQ(true, bpm->UnpinPage(flood_page_id, false));
                        // If the page is still in buffer pool then put it in free list,
                        // else also we are happy
                        EXPECT_EQ(true, bpm->DeletePage(flood_page_id));
                    }
                }
            }));
        }
        ...
        for (int j = 0; j < buffer_pool_size; j++) {
            EXPECT_EQ(true, bpm->DeletePage(page_ids[j]));
        }
    }
